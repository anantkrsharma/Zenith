...
export const updateIndustryInsights = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const isRateLimitError = (err: unknown): boolean => {
      ...
      const status =
        providerError.status || providerError.code || providerError.error?.code;
      return (
        status === 429 || status === "429" || status === "RESOURCE_EXHAUSTED"
      );
    };
    ...
    const retryWithBackoff = async <T>(
      ...
      throw lastErr instanceof Error
        ? lastErr
        : new Error("Unknown error in retry");
    };
    ...
    for (const { industry } of industries) {
      ...
      const runGemini = async ({ prompt }: { prompt: string }) => {
        ...
        const text: string = result.text ?? ""; // ensure string
        return { text };
      };
      ...
      try {
        ...
        let insights: import("@/lib/ai-schema").IndustryInsights;
        try {
          ...
          insights = industryInsightsSchema.parse(JSON.parse(cleanedText));
        } catch (parseErr) {
        ...
        if (isRateLimitError(err)) {
          ...
          await step.sleep(
            `post-rate-limit-jitter-${industry}`,
            `${Math.round(jitter)}ms`,
          );
        }
      }
    }
