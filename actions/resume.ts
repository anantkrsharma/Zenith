...
export async function improveWithAI({
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      console.error(
        "An unknown error occurred while generating AI job description",
      );
      throw new Error(
        "An unknown error occurred while generating AI job description",
      );
  ...
  try {
    ...
    if (workExpArr && workExpArr.length > 0) {
      const workExpSection = workExpArr
        .map(
          (w) =>
            `${w.title} at ${w.organization} (${w.startDate} - ${w.current ? "Present" : w.endDate}): ${w.description}`,
        )
        .join("; ");
    ...
    if (projectsArr && projectsArr.length > 0) {
      const projectsSection = projectsArr
        .map(
          (p) =>
            `${p.title} (${p.startDate} - ${p.current ? "Present" : p.endDate}): ${p.description}`,
        )
        .join("; ");
    ...
    if (educationArr && educationArr.length > 0) {
      const educationSection = educationArr
        .map(
          (e) =>
            `${e.title} at ${e.institute} (${e.startDate} - ${e.current ? "Present" : e.endDate})${e.description ? ": " + e.description : ""}`,
        )
        .join("; ");
    ...
    if (error instanceof Error) {
      console.error(
        "Error while generating AI professional summary",
        error.message,
      );
      throw new Error("Error while generating AI professional summary");
    } else {
      console.error(
        "An unknown error occurred while generating AI professional summary",
      );
      throw new Error(
        "An unknown error occurred while generating AI professional summary",
      );
