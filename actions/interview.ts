...
import { interviewQuestionsSchema } from "@/lib/ai-schema";
...
export async function generateInterviewQuestions() {
  try {
    ...
    const interviewQuestions = interviewQuestionsSchema.parse(
    ...
    if (error instanceof Error) {
      console.error(
        "Error while generating the interview questions:",
        error.message,
      );
...
export async function saveInterviewAssessment(
  ...
  try {
    ...
    const wrongAnswers = questionData.filter(
      (quesData) => quesData.isCorrect === false,
    );

    let improvementTip = "";

    if (wrongAnswers.length > 0) {
      const wrongAnswerDetails = wrongAnswers
        .map(
          (wrongAns) => `
    ...
    return { ...interviewAssessment, questions: questionData };
  } catch (error) {
    if (error instanceof Error) {
...
export async function getInterviewAssessments() {
  try {
    ...
    if (error instanceof Error) {
      ...
      console.error(
        "An unknown error occurred while fetching interview assessments.",
      );
      throw new Error(
        "An unknown error occurred while fetching interview assessments.",
      );
