...
export const workExpSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.endDate) {
      ...
      const isRecent =
        ed.year === currentYM.year &&
        (ed.month === currentYM.month || ed.month === currentYM.month - 1);
  ...
  .superRefine((data, ctx) => {
    ...
    if (!data.current && !data.endDate) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["endDate"],
        message: "End date is required unless you're currently studying here",
      });
    }
